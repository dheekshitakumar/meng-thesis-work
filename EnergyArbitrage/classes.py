...
class DQN(nn.Module):
    ...
    def select_action(self, state):
        with torch.no_grad():
            ...
            #Q = self.remove_invalid(state,Q)
            action_index = torch.argmax(Q, dim=1)
        return action_index
    
    def remove_invalid(self,state,action_probs):
        soc = state[:,0]
        soc_0 = torch.where(soc <= 0.5, -1000, 1)
        soc_1 = torch.where(soc < 0.1, -1000, 1)
        soc_3 = torch.where(soc > 0.9, -1000, 1)
        soc_4 = torch.where(soc >= 0.5, -1000, 1)
        soc_2 = torch.ones(soc.shape)
        mask = torch.stack([soc_0, soc_1, soc_2, soc_3, soc_4], dim=1)
