...
class BatteryEnv(gym.Env):
    def __init__(self, df, Eess=1, SOCmax=1, SOCmin=0, nu=0.9, PowStBy=0):
        ...
        self.action_space = spaces.Box(low=np.ones([1,1])*-Eess/2, high=np.ones([1,1])*Eess/2, dtype=np.float16)

        lows_array = np.zeros([1,25])
        highs_array = np.ones([1,25])
        lows_array[0] = self.SOCmin
        highs_array = highs_array*(max(self.df['total_lmp_rt'])+1)
...
class BatteryEnvDeg(gym.Env):
    ...
    def _take_action(self, action, ct):
        ...
        # #if Power is negative, it's charging. If Power is positive, it's discharging
        # Pow_high = min(no_loss_high, self.Eess)
        # Pow_low = max(no_loss_low, -self.Eess)
        ...
        if action_type == "rest":
            ...
            # print(ct, Pow, self.alpha,P_max+0.0001)
            deg = self.deg()
            Rt = self.scaling_factor*(ct*(Pow_grid/P_max)) + ((flag==1)* -10) - deg*self.degcost
